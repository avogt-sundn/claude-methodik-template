...
- Backend in `src/backend/` (Java, Spring Boot)
- Frontend in `src/frontend/` (Angular)
...
3. **Domain boundary** — Changes in different domain folders should be separate unless tightly coupled
...
### Wenn Java-Dateien geändert wurden:
```bash
mvn compile   # Kompilierung prüfen
mvn test      # Tests ausführen
```

### Wenn Angular-Dateien geändert wurden:
```bash
...
[Optional: BREAKING CHANGE, closes #issue]
```

**Types**: `feat`, `fix`, `refactor`, `chore`, `docs`, `test`, `ci`, `build`, `style`, `perf`

**Scopes**: `backend`, `frontend`, `docs`, `tests`, `build`, `chore`, `erkundungen`, `tickets`, `agents`
