...
```
Exportiere docs/acl/ARCHITEKTUR.md als HTML und PDF
```

oder mehrere auf einmal:

```
Exportiere alle Markdown-Dateien in docs/ als HTML und PDF
```
...
**Beispiele**:
```bash
node .claude/tools/domain-to-pdf.js erkundungen/wie-modelliert-man-das-datenmodell/DOMAIN.md
node .claude/tools/domain-to-pdf.js docs/architektur/UEBERSICHT.md /tmp/architektur.pdf
```

## Voraussetzungen (im DevContainer einrichten)

| Was | Befehl |
|---|---|
| `mmdc` (mermaid-cli) | `npm install -g @mermaid-js/mermaid-cli` |
| `marked` + `playwright` | `cd /tmp && npm install marked playwright` |
| Puppeteer-Konfig | `echo '{"executablePath":"{{CHROMIUM_PATH}}","args":["--no-sandbox","--disable-setuid-sandbox"]}' > /tmp/puppeteer-cfg.json` |

> `{{CHROMIUM_PATH}}` ersetzen durch den tatsächlichen Chromium-Pfad im DevContainer, z. B. `/ms-playwright/chromium-1208/chrome-linux/chrome`.

Falls `/tmp/puppeteer-cfg.json` fehlt (nach Container-Neustart):
```bash
echo '{"executablePath":"{{CHROMIUM_PATH}}","args":["--no-sandbox","--disable-setuid-sandbox"]}' > /tmp/puppeteer-cfg.json
```
