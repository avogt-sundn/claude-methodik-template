...
3. **Derive tickets per milestone.** Each ticket is one coherent, independently implementable change. Tickets within a milestone may parallelize; milestones are sequential. Assign each ticket an area (`backend`, `frontend`, `erkundungen`, `tickets`, `docs`).
